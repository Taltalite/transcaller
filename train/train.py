import torch
import torch.nn as nn
import torch.optim as optim
from torch.utils.data import Dataset, DataLoader, random_split, Subset
import h5py
import os
import time
import argparse
from tqdm import tqdm
import numpy as np
import random

# --- 关键导入 ---
import sys
try:
    sys.path.append('/home/lijy/workspace/')
    from my_basecaller.model.transcaller_light import TranscallerLight
except ImportError:
    print("="*80)
    print("错误: 无法导入 'TranscallerLight'。")
    print("="*80)
    exit(1)

def set_seed(seed: int = 42):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed(seed)
        torch.cuda.manual_seed_all(seed)
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False
    os.environ['PYTHONHASHSEED'] = str(seed)
    print(f"Global seed set to {seed}")

# ==========================================================================================
# 步骤 1: HDF5 数据集类
# ==========================================================================================

class BasecallerHDF5Dataset(Dataset):
    """
    (原始版本 - 磁盘读取)
    用于读取 HDF5 格式的 Basecaller 数据集的自定义 Dataset。
    """
    def __init__(self, h5_file_path):
        super().__init__()
        self.h5_file_path = h5_file_path
        
        try:
            with h5py.File(self.h5_file_path, 'r') as f:
                self.dataset_len = f['event'].shape[0]
                assert 'label' in f, "HDF5 文件中未找到 'label' 键"
                assert 'label_len' in f, "HDF5 文件中未找到 'label_len' 键"
        except Exception as e:
            print(f"打开或验证 HDF5 文件 {h5_file_path} 失败: {e}")
            raise
            
        self.h5_file = None
        self.pid = None 

    def __len__(self):
        return self.dataset_len

    def __getitem__(self, idx):
        if self.pid != os.getpid():
            if self.h5_file:
                self.h5_file.close() 
            self.h5_file = h5py.File(self.h5_file_path, 'r')
            self.pid = os.getpid()
            
        event = self.h5_file['event'][idx] 
        label = self.h5_file['label'][idx] 
        label_len = self.h5_file['label_len'][idx]
        
        event_tensor = torch.from_numpy(event).float()
        label_tensor = torch.from_numpy(label).long()
        # 🚀 关键修复:
        # 将 HDF5 中存储的 -1 (空白)  
        # 转换为训练脚本期望的空白 ID 4 。
        label_tensor[label_tensor == -1] = 4
        label_len_tensor = torch.tensor(label_len).long()
        
        return event_tensor, label_tensor, label_len_tensor

# 🚀 ==========================================================================================
# 🚀 步骤 1 (新增): 快速内存加载数据集
# 🚀 ==========================================================================================

class FastInMemoryHDF5Dataset(Dataset):
    """
    (新版本 - 快速内存加载)
    通过一次大型的顺序读取，将 HDF5 文件的 *前 N 个* 样本快速加载到 RAM 中。
    """
    def __init__(self, h5_file_path, num_samples_to_load):
        super().__init__()
        
        print(f"🚀 开始将 *前 {num_samples_to_load}* 个样本从 {h5_file_path} 顺序加载到内存...")
        
        try:
            with h5py.File(h5_file_path, 'r') as f:
                # 1. 一次性顺序读取 N 个样本 (HDF5 切片)
                # 这是最关键的改动
                print("   (1/3) 正在加载 'event'...")
                events_np = f['event'][0:num_samples_to_load]
                
                print("   (2/3) 正在加载 'label'...")
                labels_np = f['label'][0:num_samples_to_load]
                
                print("   (3/3) 正在加载 'label_len'...")
                label_lens_np = f['label_len'][0:num_samples_to_load]

            # 2. 将 Numpy 数组转换为 Tensors
            print("   正在将数据转换为 Tensors...")
            self.events = torch.from_numpy(events_np).float()
            self.labels = torch.from_numpy(labels_np).long()
            self.label_lens = torch.from_numpy(label_lens_np).long()
            
            # 🚀 关键修复:
            # 将 HDF5 中存储的 -1 (空白)  
            # 转换为训练脚本期望的空白 ID 4 。
            print(f" 正在将内存中所有 -1 标签值转换为空白符 (4)...")
            self.labels[self.labels == -1] = 4
            print(f" 转换完成。")
            
            self.dataset_len = self.events.shape[0]
            print(f"🚀 数据已全部加载到内存。总样本数: {self.dataset_len}")
            
        except Exception as e:
            print(f"加载数据到内存时出错: {e}")
            raise

    def __len__(self):
        return self.dataset_len

    def __getitem__(self, idx):
        # 直接从 RAM 返回，速度极快
        return self.events[idx], self.labels[idx], self.label_lens[idx]


# ==========================================================================================
# 步骤 2: 训练和验证函数 (与您提供的代码相同)
# ==========================================================================================
# (train_one_epoch 和 validate 函数保持不变，此处省略以节省空间)
static_printed = False
def train_one_epoch(model, criterion, optimizer, data_loader, device, output_len, scheduler_warmup, warmup_steps, global_step):
    model.train() # 设置为训练模式
    total_loss = 0.0
    progress_bar = tqdm(data_loader, desc='[训练]', leave=False)
    
    # 🚀 引用全局标志
    global static_printed
    
    for events, labels, label_lengths in progress_bar:
        events = events.to(device, non_blocking=True) # (B, 1, 2048)
        labels = labels.to(device, non_blocking=True) # (B, 200)
        label_lengths = label_lengths.to(device, non_blocking=True) # (B,)
        
        optimizer.zero_grad()
        log_probs = model(events) # (T, B, C) -> (420, B, 5)
        
        batch_size = events.shape[0]
        input_lengths = torch.full(size=(batch_size,), fill_value=output_len, dtype=torch.long, device=device)
        
        # 🚀 ==========================================================
        # 🚀 关键诊断：在计算 Loss 之前打印所有输入
        # 🚀 ==========================================================
        if not static_printed:
            print("\n" + "="*80)
            print("--- 调试：即将送入 CTCLoss 的张量 (仅打印一次) ---")
            print(f"  log_probs.shape:   {log_probs.shape}")
            print(f"  labels.shape:      {labels.shape}")
            print(f"  input_lengths.shape: {input_lengths.shape}")
            print(f"  label_lengths.shape: {label_lengths.shape}")
            
            print("\n  --- 关键检查 ---")
            print(f"  labels (Min / Max):       {labels.min().item()} / {labels.max().item()}")
            print(f"  label_lengths (Min / Max): {label_lengths.min().item()} / {label_lengths.max().item()}")
            
            print(f"\n  input_lengths (前5个): {input_lengths[:5]}")
            print(f"  label_lengths (前5个): {label_lengths[:5]}")
            
            # 检查标签中是否有非法值（-1）
            if labels.min().item() < 0:
                print("  🔥 致命错误：在训练循环中发现 'labels' 包含负值！")
                print("     数据转换失败。")
            elif labels.max().item() > 4:
                print("  🔥 致命错误：在训练循环中发现 'labels' 包含 > 4 的值！")
                print("     (0=A, 1=C, 2=G, 3=T, 4=Blank)")
            elif label_lengths.min().item() == 0:
                 print("  🔥 致命错误：在训练循环中发现 'label_lengths' 包含 0！")
                 print("     CTCLoss 不允许 0 长度的标签。")
            else:
                 print("  ✅ 数据看起来有效。如果仍然失败，问题在模型。")

            print("="*80 + "\n")
            static_printed = True
        # 🚀 ==========================================================
        
        loss = criterion(log_probs, labels, input_lengths, label_lengths)
                
        if torch.isinf(loss):
            print("警告: 遇到 inf 损失，跳过此 batch。")
            continue
            
        loss.backward()
        # ==========================================================
        # 添加梯度裁剪
        # (在 loss.backward() 之后, optimizer.step() 之前)
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        # ==========================================================
        optimizer.step()
        
        if global_step < warmup_steps:
            scheduler_warmup.step()
        global_step += 1
        
        total_loss += loss.item()
        progress_bar.set_postfix(loss=f'{loss.item():.4f}')
        
        current_lr = optimizer.param_groups[0]['lr']
        progress_bar.set_postfix(loss=f'{loss.item():.4f}', lr=f'{current_lr:.1e}')
            
    avg_loss = total_loss / len(data_loader)
    return avg_loss, global_step


def validate(model, criterion, data_loader, device, output_len):
    model.eval() # 设置为评估模式
    total_loss = 0.0
    progress_bar = tqdm(data_loader, desc='[验证]', leave=False)
    
    with torch.no_grad(): # 关闭梯度计算
        for events, labels, label_lengths in progress_bar:
            events = events.to(device, non_blocking=True)
            labels = labels.to(device, non_blocking=True)
            label_lengths = label_lengths.to(device, non_blocking=True)
            
            log_probs = model(events)
            
            batch_size = events.shape[0]
            input_lengths = torch.full(size=(batch_size,), fill_value=output_len, dtype=torch.long, device=device)
            
            loss = criterion(log_probs, labels, input_lengths, label_lengths)
            
            if not torch.isinf(loss):
                total_loss += loss.item()
                progress_bar.set_postfix(loss=f'{loss.item():.4f}')
            
    avg_loss = total_loss / len(data_loader)
    return avg_loss

# ==========================================================================================
# 步骤 3: 主函数 (🚀 此部分已修改)
# ==========================================================================================

def main(args):
    
    # --- 1. 设置环境 ---
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"使用设备: {device}")
    os.makedirs(args.checkpoint_dir, exist_ok=True)
    
    MODEL_DOWNSAMPLE_RATIO = 4 
    true_output_len = args.input_len // MODEL_DOWNSAMPLE_RATIO 
    
    if args.output_len != true_output_len:
        print("="*80)
        print(f"警告: --output-len 参数 ({args.output_len}) 与模型实际下采样不符。")
        print(f"       模型  (s=2, s=2) 产生 {true_output_len} 的输出长度。")
        print(f"       将强制使用 {true_output_len} 进行 CTC 损失计算。")
        print("="*80)
        
        # ‼️ 必须覆盖：我们必须使用正确的长度 (512) 
        #    来喂给 train_one_epoch 和 validate
        args.output_len = true_output_len
    
    # --- 2. 准备数据集 (🚀 此部分逻辑已更新) ---
    print("加载数据集中...")
    
    # 首先，我们需要知道数据集总大小
    try:
        with h5py.File(args.data_file, 'r') as f:
            total_dataset_len = f['event'].shape[0]
    except Exception as e:
        print(f"打开 HDF5 文件 {args.data_file} 失败: {e}")
        return

    # 确定要处理的样本数
    if args.num_samples > 0:
        num_samples = min(total_dataset_len, args.num_samples)
    else:
        num_samples = total_dataset_len

    # 🚀 (Utils 1) - 根据是否加载到内存，选择不同的 Dataset
    if args.load_in_ram:
        # 模式一: 快速加载 *前 N 个* 样本到 RAM
        print("🚀 启动 [快速内存加载] 模式...")
        dataset_to_split = FastInMemoryHDF5Dataset(args.data_file, num_samples)
    else:
        # 模式二: 传统磁盘 I/O (随机子集)
        print("🚀 启动 [磁盘读取] 模式...")
        if args.num_samples > 0:
             print(f"使用 {num_samples} 个 *随机* 样本 (总共 {total_dataset_len})...")
             # 1. 创建完整 HDF5 数据集的句柄
             full_dataset_ondisk = BasecallerHDF5Dataset(args.data_file)
             # 2. 生成随机索引
             g = torch.Generator().manual_seed(args.seed)
             indices = torch.randperm(total_dataset_len, generator=g)[:num_samples]
             # 3. 使用 Subset 进行包装 (这将导致随机 I/O)
             dataset_to_split = Subset(full_dataset_ondisk, indices)
        else:
            print(f"使用完整数据集 ({total_dataset_len} 样本)...")
            dataset_to_split = BasecallerHDF5Dataset(args.data_file)


    # (Utils 2) - 划分训练集和验证集
    val_size = int(len(dataset_to_split) * args.val_split)
    train_size = len(dataset_to_split) - val_size
    
    train_dataset, val_dataset = random_split(
        dataset_to_split, 
        [train_size, val_size],
        generator=torch.Generator().manual_seed(args.seed) # 保证划分可复现
    )
    
    print(f"  训练集大小: {train_size}")
    print(f"  验证集大小: {val_size}")

    # (Utils 3) - 创建 Dataloader
    # 🚀 注意: 即使在内存模式下，num_workers > 0 也是有益的，
    # 因为它会在后台进程中执行 batch 的组合和 pin_memory 操作。
    train_loader = DataLoader(
        train_dataset,
        batch_size=args.batch_size,
        shuffle=False,
        # num_workers=args.num_workers,
        num_workers=0,
        pin_memory=True,  
        drop_last=True 
    )
    
    val_loader = DataLoader(
        val_dataset,
        batch_size=args.batch_size,
        shuffle=False,
        # num_workers=args.num_workers,
        num_workers=0,
        pin_memory=True
    )

    # --- 3. 初始化模型、损失函数和优化器 ---
    # (此部分与您提供的代码相同)
    
    print("初始化模型...")
    model = TranscallerLight(
        input_length=args.input_len,   # (来自数据, 2048)
        output_length=args.output_len, # (模型设计, 512)
        num_classes=args.num_classes,  # (A,C,G,T,blank = 5)
        embed_dim=args.embed_dim,
        depth=args.depth,
        num_heads=args.num_heads,
        drop_path_rate=args.drop_path
    ).to(device)
    
    print("\n" + "="*80)
    print("--- 调试：正在打印实际运行的模型架构 ---")
    print("--- 请仔细检查下面的 [Head] 模块 ---")
    print("--- 它是否仍然包含 'AdaptiveAvgPool1d'？ ---")
    print(model)
    print("="*80 + "\n")
    
    print(f"模型参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad):,}")

    criterion = nn.CTCLoss(blank=args.blank_id, zero_infinity=True)
    optimizer = optim.AdamW(model.parameters(), lr=args.lr)
    
    # 调度器 1: 线性预热 (在 warmup_steps 步内, LR 从 1e-7 -> args.lr)
    # 注意: LinearLR 是按 "step" 更新的
    steps_per_epoch = len(train_loader)
    warmup_steps = steps_per_epoch 
    print(f" 每个 Epoch 步数: {steps_per_epoch}")
    print(f" (关键) Warmup 步数 (1 epoch): {warmup_steps}")
    scheduler_warmup = optim.lr_scheduler.LinearLR(
        optimizer, 
        start_factor=1e-7 / args.lr, # 从一个极小值开始
        end_factor=1.0, 
        total_iters=warmup_steps
    )
    
    # 调度器 2: 平台衰减 (保持不变)
    # 注意: ReduceLROnPlateau 是按 "epoch" (基于 val_loss) 更新的
    scheduler_plateau = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'min', 
        patience=args.scheduler_patience, 
        factor=0.5, 
        verbose=True
    )

    # --- 4. 训练循环 ---
    # (此部分与您提供的代码相同)
    print(f"开始训练... 共 {args.epochs} 轮")
    best_val_loss = float('inf') 
    global_step = 0 # 跟踪总步数

    for epoch in range(1, args.epochs + 1):
        print(f"\n--- Epoch {epoch}/{args.epochs} ---")
        
        start_time = time.time()
        
        # train_loss = train_one_epoch(model, criterion, optimizer, train_loader, device, args.output_len)
        # val_loss = validate(model, criterion, val_loader, device, args.output_len)
        
        train_loss, global_step = train_one_epoch(
            model, criterion, optimizer, train_loader, device, args.output_len,
            scheduler_warmup, warmup_steps, global_step # 传入新参数
        )
        
        # 2. (不变) 验证
        val_loss = validate(model, criterion, val_loader, device, args.output_len)
        
        # 3. (修改) 只在 Warmup 结束后才使用 Plateau 调度器
        if global_step >= warmup_steps:
             scheduler_plateau.step(val_loss) # 更新学习率
        
        elapsed = time.time() - start_time
        print(f"Epoch {epoch} 完成. 耗时: {elapsed:.2f}s")
        print(f"  [总结] 训练损失: {train_loss:.4f} | 验证损失: {val_loss:.4f}")
        
        # (Utils 3) - 模型保存
        save_path_latest = os.path.join(args.checkpoint_dir, "model_latest.pth")
        torch.save({
            'epoch': epoch,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'scheduler_plateau_state_dict': scheduler_plateau.state_dict(),
            'scheduler_warmup_state_dict': scheduler_warmup.state_dict(),
            'val_loss': val_loss,
        }, save_path_latest)
        
        if val_loss < best_val_loss:
            best_val_loss = val_loss
            save_path_best = os.path.join(args.checkpoint_dir, "model_best.pth")
            torch.save(model.state_dict(), save_path_best)
            print(f" (新最佳模型! 验证损失: {val_loss:.4f}, 已保存至 {save_path_best})")
            
    print("\n" + "="*80)
    print("训练完成!")
    print(f"最佳验证损失: {best_val_loss:.4f}")
    print(f"最佳模型保存在: {os.path.join(args.checkpoint_dir, 'model_best.pth')}")

# ==========================================================================================
# 步骤 4: Argparse 命令行参数
# ==========================================================================================

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="训练 Melchior Basecaller 模型")
    
    # --- 数据和路径参数 ---
    parser.add_argument('--data-file', type=str, required=True,
                        help="HDF5 数据集文件路径")
    parser.add_argument('--checkpoint-dir', type=str, default="./checkpoints",
                        help="保存模型 checkpoint 的目录")
    
    # --- (Utils) 数据集控制 ---
    parser.add_argument('--num-samples', type=int, default=-1,
                        help="要使用的训练样本数量。-1 表示使用全部。 (默认: -1)")
    parser.add_argument('--val-split', type=float, default=0.1,
                        help="用于验证集的比例 (例如 0.1 表示 10%) (默认: 0.1)")
    
    # 🚀 --- (更新) IO 优化 ---
    parser.add_argument('--load-in-ram', action='store_true',
                        help="将 *前 N 个* 样本快速加载到 RAM 中以加速训练 (推荐)")
    
    # --- 训练超参数 ---
    parser.add_argument('--epochs', type=int, default=20,
                        help="训练轮数 (默认: 20)")
    parser.add_argument('--batch-size', type=int, default=128,
                        help="批量大小 (默认: 128)")
    parser.add_argument('--lr', type=float, default=1e-4,
                        help="学习率 (默认: 1e-4)")
    parser.add_argument('--num-workers', type=int, default=8,
                        help="DataLoader 使用的进程数 (默认: 8)")
    parser.add_argument('--seed', type=int, default=42,
                        help="随机种子 (默认: 42)")
    parser.add_argument('--scheduler-patience', type=int, default=3,
                        help="LR 调度器等待的轮数 (默认: 3)")

    # --- 模型架构参数 (必须与数据和模型设计匹配) ---
    parser.add_argument('--input-len', type=int, default=2048,
                        help="输入信号序列长度 (默认: 2048)")
    parser.add_argument('--output-len', type=int, default=512,
                        help="模型输出序列长度 (默认: 420)")
    parser.add_argument('--num-classes', type=int, default=5,
                        help="类别数 (A,C,G,T,blank) (默认: 5)")
    parser.add_argument('--blank-id', type=int, default=4,
                        help="CTCLoss 的空白标签 ID (默认: 4)")
    
    # --- (可选) Transcaller 内部参数 ---
    parser.add_argument('--embed-dim', type=int, default=768,
                        help="Transformer 嵌入维度 (默认: 768)")
    parser.add_argument('--depth', type=int, default=12,
                        help="Transformer 层数 (默认: 12)")
    parser.add_argument('--num-heads', type=int, default=6,
                        help="Transformer 注意力头数 (默认: 6)")
    parser.add_argument('--drop-path', type=float, default=0.1,
                        help="随机深度概率 (默认: 0.1)")
    
    
    args = parser.parse_args()
    
    if args.seed >= 0:
        set_seed(seed=args.seed)
    
    # 打印所有配置
    print("="*80)
    print("训练配置:")
    for k, v in vars(args).items():
        print(f"  {k}: {v}")
    print("="*80)
    
    main(args)