...
It uses mv/ts tags in the BAM to align signal to reference, applies robust scaling
(Median/MAD) with clipping, and writes chunks.npy / references.npy /
reference_lengths.npy compatible with Bonito loaders.
"""
...
def typical_indices(x: np.ndarray, n: float = 2.5) -> np.ndarray:
    ...
    low = center - n * scale
    high = center + n * scale
    idx, = np.where((low < x) & (x < high))
    return idx


def robust_scale(signal: np.ndarray) -> np.ndarray:
...
def process_task(task: TaskData) -> Tuple[List[Sample], Dict[str, int]]:
    ...
    for win_start in range(0, pa_signal.shape[0] - chunk_len + 1, chunk_stride):
        ...
        normalized_signal = robust_scale(signal_window)
...
def main() -> None:
    ...
    print("[4/5] Merging to final .npy files (with filtering & shuffling)...")
