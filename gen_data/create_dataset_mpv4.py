...
This script recreates the --save-ctc dataset output without re-running basecalling.
It uses mv/ts tags in the BAM to align signal to reference, applies Bonito-style 
Quantile scaling, and writes chunks.npy / references.npy / reference_lengths.npy 
compatible with Bonito loaders.
"""
...
EPS = 1e-5
# MAD_SCALE Removed: Bonito uses standard Quantile normalization logic
...
def typical_indices(x: np.ndarray, n: float = 2.5) -> np.ndarray:
    """
    Modified to match Bonito's io.py logic exactly.
    Bonito uses Mean and Std for outlier filtering, not Median/MAD.
    """
    if x.size == 0:
...
def compute_bonito_norm(signal: np.ndarray) -> Tuple[float, float]:
    """
    Calculate shift and scale using Bonito's default Quantile Scaling strategy.
    Params based on __default_norm_params__ in bonito/reader.py
    """
    # Bonito defaults: quantile_a=0.2, quantile_b=0.9
    qa, qb = np.quantile(signal, [0.2, 0.9])
    
    # Bonito defaults: shift_multiplier=0.51, scale_multiplier=0.53
    shift = 0.51 * (qa + qb)
    scale = 0.53 * (qb - qa)
    
    # Avoid division by zero, though unlikely with PA data
    if scale < EPS:
        scale = 1.0
...
def process_task(task: TaskData) -> Tuple[List[Sample], Dict[str, int]]:
    ...
    try:
        # Fetch the FULL calibrated (pA) signal for the read
    ...
    # --- Bonito Normalization Change ---
    # Bonito calculates normalization parameters on the FULL read signal,
    # before chunking.
    global_shift, global_scale = compute_bonito_norm(pa_signal)
    # -----------------------------------
    ...
    for win_start in range(0, pa_signal.shape[0] - chunk_len + 1, chunk_stride):
        ...
        # --- Bonito Normalization Application ---
        # Apply the GLOBAL shift/scale to the window
...
def merge_chunks_to_final(
    ...
    # Filter indices using Bonito-style (Mean/Std) outlier detection
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--chunk-len", type=int, default=4000)
