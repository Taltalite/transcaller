...
from statsmodels.robust import mad as median_absolute_deviation
...
def process_task(task: TaskData) -> Tuple[List[Sample], Dict[str, int]]:
    ...
    for win_start in range(0, pa_signal.shape[0] - chunk_len + 1, chunk_stride):
        ...
        # [CRITICAL FIX] Use Robust Scaling (Median/MAD) instead of Mean/Std
        # Bonito/Dorado models thrive on this scaling
        normalized_signal = robust_scale(signal_window)

        # 截断极端值 (Clip)，防止梯度爆炸，通常截断在 [-5, 5]
        normalized_signal = np.clip(normalized_signal, -5.0, 5.0)
